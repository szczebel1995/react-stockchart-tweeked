...
function renderSVG(props) {
	...
	console.log("RENDER SVG PROPS", props);
...
function helper(props) {
	...
	if (isPriceHighlight) rectWidth = 50;
...
function drawOnCanvas(ctx, props) {
	...
	if ((0, _utils.isDefined)(edge.coordinateBase)) {
		...
		if (edge.line.isPriceHighlight) {
			//Thin line above or below box
			ctx.beginPath();
			var grd = ctx.createLinearGradient(x - 105, 0, x - 105 + 150, 0);
			var lineYPos = edge.line.islowHighlight ? y - 8 : y + 33;

			ctx.fillStyle = edge.line.highlightColorGradient[0];
			roundRect(ctx, x - 140, lineYPos, 190, 4, 1);
			ctx.fill();
		}
		...
		if (edge.orient === "right" && !edge.line.isEdge && !edge.line.isPriceHighlight) {
			...
			ctx.closePath();
		} else if (!edge.line.isPriceHighlight) {
			//last price box
			/* ctx.shadowBlur = edge.line.shadowBlur;
				 ctx.shadowColor = edge.line.shadowColor;*/
			...
			ctx.fillStyle = "#25496C";
			ctx.shadowBlur = false;
			ctx.shadowColor = "#25496C";
			/* ctx.fillStyle = grd*/
			...
			grd.addColorStop(1, edge.line.highlightColorGradient[1]);
			ctx.fillStyle = grd;
			roundRect(ctx, x - 140, y - 6, 190, 41, 0); //SquareBox
		}
		ctx.fill();
		...
		if (!edge.line.isEdge && !edge.line.isPriceHighlight) {
			//Price coordinate text.
			ctx.fillStyle = edge.coordinate.textFill;
			ctx.textAlign = edge.coordinate.textAnchor === "middle" ? "end" : edge.coordinate.textAnchor;
			...
			//Edge coordinate text (current price along y axis.)
			/* ctx.fillStyle = "#56b3e2";*/

			var textY = edge.coordinate.edgeYText - 4;
			ctx.fillStyle = "white";
			ctx.textAlign =
			...
			ctx.font = "500 13px Ubuntu Helvetica Neue, Helvetica, Arial, sans-serif";
			console.log("PADDING", {edge: edge, padding: edge.textPadding});
			ctx.fillText(
				edge.line.highlightTitle,
				edge.coordinate.edgeXText - 120 + edge.coordinate.textPadding,
				edge.coordinate.edgeYText - 4
			);
			ctx.fillStyle = "#e8e9ea";
			ctx.font = "600 16px Ubuntu Helvetica Neue, Helvetica, Arial, sans-serif";
			ctx.fillText(
				edge.line.highlightPrice,
				edge.coordinate.edgeXText - 145,
				edge.coordinate.edgeYText + 13
			);
		}
	}
}
