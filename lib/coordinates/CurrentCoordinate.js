...
var CurrentCoordinate = function (_Component) {
	...
	_createClass(CurrentCoordinate, [{
		...
		value: function drawOnCanvas(ctx, moreProps) {
			...
			ctx.shadowBlur = 0;
			ctx.lineJoin = "round";
			ctx.strokeStyle = "#519DE8";
			ctx.lineWidth = 2;
			ctx.fillStyle = "#12263b";
			var halfScreenWidth = circle.screenWidth / 2;
			if(circle.x > halfScreenWidth){//Draw Quote boxes
				ctx.beginPath();
				roundedQuoteBoxRight(ctx, circle.x - 133, circle.y - 13, 120, 26, 5, 5, 0.5);
				ctx.fill();
				ctx.stroke()
				ctx.font = '500 11.5pt Ubuntu, Helvetica Neue, Helvetica, Arial, sans-serif';
				ctx.fillStyle = "#9AC2E9";
				ctx.fillText((circle.close.close).toFixed(5) + " " + circle.acronym, circle.x - 116, circle.y + 4.5);
				...
				// draw left quote box
				ctx.beginPath();
				roundedQuoteBoxLeft(ctx, circle.x + 13, circle.y - 13, 120, 26, 5, 5, 0.5);
				ctx.fill();
				ctx.stroke()
				ctx.font = '500 11.5pt Ubuntu, Helvetica Neue, Helvetica, Arial, sans-serif';
				ctx.fillStyle = "#9AC2E9";
				ctx.fillText((circle.close.close).toFixed(5) + " " + circle.acronym, circle.x + 29, circle.y + 4.5);
				//USD todo
				// ctx.font = '9pt Ubuntu, Helvetica Neue, Helvetica, Arial, sans-serif';
				// ctx.fillStyle = "rgb(51, 105, 133)";
