...
var PriceCoordinate = (function(_Component) {
	...
	function PriceCoordinate(props) {
		...
		var _this = _possibleConstructorReturn(
			this,
			(PriceCoordinate.__proto__ || Object.getPrototypeOf(PriceCoordinate)).call(this, props)
		);
...
PriceCoordinate.propTypes = {
	...
	textPadding: _propTypes2.default.number,
...
function helper(props, moreProps) {
	console.log("PRICE COORDINATE HELPER", arguments);
	...
	if (priceCoordinates != undefined && priceCoordinates.length != 0 && price !== 0) {
		if (priceIndex !== undefined) {
			var highlightPrice = firstPriceRef - (upperPrice - lowerPrice) * 0.027 * priceIndex;
			if (
				highlightPrice < upperPrice + priceShowTolerance &&
				highlightPrice > lowerPrice - priceShowTolerance &&
				priceIndex !== undefined
			) {
				y =
					firstPriceRef / rangeSlope +
					(lowerYValue - lowerPrice / rangeSlope) +
					rectHeight * priceIndex;
				show = true;
			} else {
				show = false;
			}
	...
	if (isPriceHighlight) {
		...
		var highlightLowerBoxPrice = highlightPrice - (upperPrice - lowerPrice) * 0.09;
