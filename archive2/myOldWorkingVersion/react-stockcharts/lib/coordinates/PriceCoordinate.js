...
function helper(props, moreProps) {
	...
	if (isPriceHighlight) {
		...
		var currentPrice = props.price//yAccessor(item);
		var islowHighlight = props.highlightPrice < yAccessor(item);
		console.log(props.highlightPrice,  yAccessor(item))
		var highlightDefaultPos = islowHighlight ? 33 : -20;
		var highlightPrice = (currentPrice) - ((upperPrice - lowerPrice) / highlightDefaultPos)

		y = highlightPrice / rangeSlope + (lowerYValue - lowerPrice / rangeSlope);

		if (y <= 1.4) {
			y = upperPrice / rangeSlope + (lowerYValue - lowerPrice / rangeSlope);
		}
		var highlightLowerBoxPrice = (highlightPrice) - (((upperPrice - lowerPrice) * .090))
		if (y >= 684.9 || highlightLowerBoxPrice <= lowerPrice) {
			y = lowerPrice / rangeSlope + (lowerYValue - lowerPrice / rangeSlope) - 25;
		}
		show = true;
